...
extend:
  while (list_len<len) {
    ...
    while (try_guesses) {
      ...
      asm volatile("mfence");
      // Access guess
      maccess((void*)(&mem[0x0]));
      maccess((void*)guess_pool[guess_index]);
      asm volatile("lfence");

      // Measure TARGET
      // time = time_mread((void*)victim); // this is the original timing, and is replaced by hackyracer
      for (int i = 0; i < 16; i++)
      {
        int temp = 1;
        int trash = 16;
        for (int volatile j = 0; j < 16; j++)
          trash--;
        asm volatile(
            "xor %%eax, %%eax\n\t"
            "add %1, %%eax\n\t"
        ...
        if (temp == 0)
          if (i == 15)
          {
            maccess((&mem[*((uint64_t *)victim)]));
          }
      }
      time = time_mread(&(mem[0x800]));
